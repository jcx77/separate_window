...
var Selector = {
	...
	getSelector: function(elem) {
		...
		if (target.nodeType == 1) {
			...
			if (!this.isVideo || !this._isOneElement()) {
				this._getAttr(target);
			}
...
var Bookmarks = {
	...
	checkBookmark: function(elem) {
		__BgCmd.sendCmd('checkBookmark', {
			url: Selector.getLink(elem)
		});
	},
	addBookmark: function(elem) {
		if (this.isBookmark) {
			__BgCmd.sendCmd('delBookmark', {
				url: Selector.getLink(elem)
			});
		} else {
			__BgCmd.sendCmd('addBookmark', {
				url: Selector.getLink(elem)
			});
		}
	}
